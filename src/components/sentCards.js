      ...
      renderCard = () => {
            ...
            if (ending === "mp4") {
                  return <div className="vidContainer"><video className="displayVid" ref={`specVideo`} src={this.state.sentCard.source}></video>
                        <button className={this.props.status === "play" ? "displayPlay" : "displayPlay hidden"} onClick={() => this.props.play(null, this.refs.specVideo)}><i className="fas fa-pause"></i></button>
                        <button className={this.props.status === "paused" ? "displayPlay" : "displayPlay hidden"} onClick={() => this.props.play(null, this.refs.specVideo)}><i className="fas fa-play"></i></button>
                  </div>
            }
            else {
